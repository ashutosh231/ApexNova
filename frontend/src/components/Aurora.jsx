...
export function Aurora({ colorStops = ['#5227FF', '#7cff67', '#5227FF'], amplitude = 1.0, blend = 0.5, speed = 1.0, time }) {
  ...
  useEffect(() => {
    ...
    const update = (t) => {
      ...
      const { time: ot, speed: spd = 1.0 } = propsRef.current;
      const ct = ot !== undefined ? ot : t * 0.01;
      if (program) {
        program.uniforms.uTime.value = ct * spd * 0.1;
